                                        ...
                                        unsigned int featureCount)
{
  ...
  double dev;
  unsigned int i;
  double sumLeftTime, sumRightTime, sumParentTime;
  ...
  if (lambdaParent == 0) {
    ...
    dev = sumParentEvent * log(lambdaParent);

    /*
    * Compute the lambda and dev for the two daughter nodes
    */
    sumLeftEvent = 0;
    sumLeftTime = 0;
    sumRightEvent = 0;
    ...
    for (i = 0; i < n; i++) {
      if (membership[i] == LEFT) {
        ...
        sumRightEvent += event[i];
        sumRightTime += time[i];
      }
    }
    lambdaLeft = sumLeftEvent / sumLeftTime;
    lambdaRight = sumRightEvent / sumRightTime;
    
    dev = -dev + sumLeftEvent * log(lambdaLeft) + sumRightEvent * log(lambdaRight);
  }
