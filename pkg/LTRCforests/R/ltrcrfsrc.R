                      ...
                      ...)
{
  ...
  nativeOutput <- tryCatch({.Call("rfsrcGrow",
                                  ...
                                  as.integer(ntree),
                                  as.integer(n),
                                  if (is.null(yvar.types)) integer(0) else as.integer(length(yvar.types)),
                                  if (is.null(yvar.types)) NULL else as.character(yvar.types),
                                  as.integer(yvar.nlevels),
                                  if (is.null(yvar)) double(0) else as.double(as.vector(yvar)),
                                  as.integer(n.xvar),
                                  as.character(xvar.types),
                                  as.integer(xvar.nlevels),
                                  as.integer(sampsize),
                                  if (is.null(samp)) integer(0) else as.integer(samp),
                                  if (is.null(case.wt)) double(0) else as.double(case.wt),
                                  as.double(split.wt),
                                  as.double(yvar.wt),
                                  as.double(xvar.wt),
                                  as.double(as.vector(xvar)),
                                  if(is.null(event.info$time.interest)) as.integer(0) else as.integer(length(event.info$time.interest)),
                                  if(is.null(event.info$time.interest)) double(0) else as.double(event.info$time.interest),
                                  as.integer(nimpute),
                                  as.integer(block.size),
                                  if (is.null(prob)) as.integer(0) else as.integer(length(prob)),
                                  if (is.null(prob)) double(0) else as.double(prob),
