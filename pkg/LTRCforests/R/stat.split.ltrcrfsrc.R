stat.split.ltrcrfsrc <- function(object, ...)
{

  ## Incoming parameter checks.  All are fatal.
  if (is.null(object)) stop("Object is empty!")
  if ((sum(inherits(object, c("ltrcrfsrc", "grow"), TRUE) == c(1, 2)) ) &&
      (sum(inherits(object, c("ltrcrfsrc", "predict"), TRUE) == c(1, 2)) ))
    stop("This function only works for objects (of subclass) of class `(rfsrc, grow)' or `(rfsrc, pred)',")
...
ltrcspParseTree <- function(recursiveObject, extendedNativeArray, xvar,
                            b,
                            depth,
                            membership,
                            ptnFlag,
  ...
  if (!recursiveObject$terminal) {
    ...
    depth <- depth + 1
    ## Parse left:
    recursiveObject <- ltrcspParseTree(recursiveObject, extendedNativeArray, xvar,
...
#stat.split <- stat.split.ltrcrfsrc
