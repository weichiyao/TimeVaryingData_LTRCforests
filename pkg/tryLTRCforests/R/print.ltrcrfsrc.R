print.ltrcrfsrc <- function(x, outcome.target = NULL, ...) {
  ## default printing
  if (sum(inherits(x, c("ltrcrfsrc", "forest"), TRUE) == c(1, 2)) == 2) {
    print.default(x)
    return()
  }
  if(sum(inherits(x, c("ltrcrfsrc", "plot.variable"), TRUE) == c(1, 2)) == 2) {
    print.default(x)
    return()
  }
  if (sum(inherits(x, c("ltrcrfsrc", "partial"), TRUE) == c(1, 2)) == 2) {
    print.default(x)
    return()
  }
  if (sum(inherits(x, c("ltrcrfsrc", "sidClustering"), TRUE) == c(1, 2)) == 2) {
    print.default(x)
    return()
  }
  ## deal with synthetic forests
  sf.flag <- FALSE
  if (sum(inherits(x, c("ltrcrfsrc", "synthetic"), TRUE) == c(1, 2)) == 2) {
    if (sum(inherits(x, c("ltrcrfsrc", "synthetic", "oob"), TRUE) == c(1, 2, 4)) != 3) {
      sf.flag <- TRUE
  ...
  if (sum(inherits(x, c("ltrcrfsrc", "subsample"), TRUE) == c(1, 4)) == 3) {
    print.subsample(x, ...)
    return()
  }
  ## is this a subsampled-bootstrap object?
  if (sum(inherits(x, c("ltrcrfsrc", "bootsample"), TRUE) == c(1, 4)) == 2) {
    print.bootsample(x, ...)
    return()
  }
  ## check that the object is interpretable
  if (sum(inherits(x, c("ltrcrfsrc", "grow"), TRUE) == c(1, 2)) != 2 &
      sum(inherits(x, c("ltrcrfsrc", "predict"), TRUE) == c(1, 2)) != 2) {
    stop("This function only works for objects of class `(ltrcrfsrc, grow)' or '(ltrcrfsrc, predict)'.")
  }
  ## are we in grow mode?
  if (sum(inherits(x, c("ltrcrfsrc", "grow"), TRUE) == c(1, 2)) == 2) {
    grow.mode <- TRUE
