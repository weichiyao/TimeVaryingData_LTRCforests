max.subtree.ltrcrfsrc <- function(object,
                              max.order = 2,
                              sub.order = FALSE,
                              conservative = FALSE,
                              ...)
{
  ...
  if (sum(inherits(object, c("ltrcrfsrc", "grow"), TRUE) == c(1, 2)) != 2    &
      sum(inherits(object, c("ltrcrfsrc", "forest"), TRUE) == c(1, 2)) != 2)
    stop("This function only works for objects of class `(ltrcrfsrc, grow)' or '(ltrcrfsrc, forest)'")
  ## Acquire the forest
  if (sum(inherits(object, c("ltrcrfsrc", "grow"), TRUE) == c(1, 2)) == 2) {
  ...
  subtree.obj <- mclapply(1:numTree, function(b) {
    ...
    recursiveObject <- ltrcrfsrcParseTree(
      recursiveObject,
      max(max.order, 1),
      sub.order,
      nativeArray,
...
ltrcrfsrcParseTree <- function(recursiveObject,
                               max.order,
                               sub.order,
                               nativeArray,
                               b,
  ...
  if (!terminalFlag) {
    ...
    if (subtreeFlag[splitParameter] == FALSE) {
      ...
      if (sub.order == TRUE) {
        ...
        recursive2Object <- ltrcrfsrcParse2Tree(recursive2Object,
                                                nativeArray,
                                                b,
                                                distance=0,
  ...
  if (terminalFlag == FALSE) {
    ...
    distance <- distance + 1
    ## Parse left:
    recursiveObject <- ltrcrfsrcParseTree(recursiveObject, max.order, sub.order, nativeArray, b, distance, subtreeFlag)
    ## Parse right:
    recursiveObject <- ltrcrfsrcParseTree(recursiveObject, max.order, sub.order, nativeArray, b, distance, subtreeFlag)
  }
    else {
...
ltrcrfsrcParse2Tree <- function(recursiveObject,
                                nativeArray,
                                b,
                                distance,
                                subtreeFlag) {
  ...
  if (terminalFlag == FALSE) {
    ## Parse left:
    recursiveObject = ltrcrfsrcParse2Tree(recursiveObject, nativeArray, b, distance, subtreeFlag)
    ## Parse right:
    recursiveObject = ltrcrfsrcParse2Tree(recursiveObject, nativeArray, b, distance, subtreeFlag)
  }
  return(recursiveObject)
...
max.subtree <- max.subtree.ltrcrfsrc
