...
Range_T_linr <- function(TALL, DIST, X, U){  # TALL = c(0,TS)
  ...
  if(DIST == "Exp"){
    ...
    R0 = exp(Beta[1]*x1 +Beta[2]*x2 + Beta[3]*x3 + Beta[4]*x4 + Beta[5]*x5 + Beta[6]*x6)
...
Timevarying_linear_gnrt <- function(N = 200, Distribution = "Exp", censor.rate = 1, npseu = 11,
                                    partial=TRUE){
  ...
  while(Count <= N){
    ...
    RT <- Range_T_linr(TALL=c(0,TS),DIST=Distribution,X=Data[Data$ID==Count,],U=u)
    t = RT$Time
    rID = RT$Row
  ...
  if (npseu == 11){
    ...
    if(censor.rate == 0){
      ...
      if(Distribution == "Exp"){
        Censor.time = rexp(N,rate = 1/92)
      }else if(Distribution == "WD"){
        Censor.time = rexp(N,rate = 1/420)
      }else if(Distribution == "WI"){
        Censor.time = rexp(N,rate = 1/89)
      }else if(Distribution == "Gtz"){
        Censor.time = rexp(N,rate = 1/62)
      }
    }else if(censor.rate == 2){
      if(Distribution == "Exp"){
        Censor.time = rexp(N,rate = 1/16)
        ...
        Censor.time = rexp(N,rate = 1/18.5)
      }
    }else{
      stop("Wrong censoring type.")
    }
