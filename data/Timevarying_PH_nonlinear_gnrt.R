...
Range_T_nonlinr <- function(TALL, DIST, X, U){  # TALL = c(0,TS)
  ...
  if(DIST == "Exp"){
    R0 = exp(-log(x6+x3/10+x5/20+1/5)-x1*(x2*2)^(4*x4))#exp(0.5*(2*x2)^(4*x4)-0.5*log(x5-x3+1/6+0.0001*(x6-2*x1))) #exp(0.5*(2*x2)^(4*x4)-0.5*log(x5-x3+1/6))
    Lambda = 0.1
    Alpha = 0
    V = 0
    Beta <- rep(0,6)
    ...
    R0 = exp(-log(x6+x3/10+x5/20+1/5)-x1*(x2*2)^(4*x4))#exp(0.5*(2*x2)^(4*x4)-0.5*log(x5-x3+1/6))#exp(x1*(2*x2)^(2*x4)-log(x6+1/6)-3*x6/x5) #exp(x1*x2-log(x3+x4)-x6/x5)
    ...
    R0 = exp(-log(x6+x3/10+x5/20+1/5)-x1*(x2*2)^(4*x4))#exp(0.3*log(x3-x1+x5/5+x6/2+1+x2-x4+1))#only log does not help#exp(0.5*x1*(2*x2)^(2*x4)+0.5*log(x3+1/6)+0.5*(x6-1)/x5) # exp(log(x6+1/5)+2.5*x1*x2^x4) 
    Lambda = 0.0025
    V = 1.8
    Alpha = 0
    Beta = rep(0,6)
    ...
    R0 = exp(-log(x6+x3/10+x5/20+1/5)-x1*(x2*2)^(4*x4))#exp(sin(x1+x2+x3+x4+x5+x6-3))#only log does not help#exp(log(x6/2+x3-x1+x5/5+1)-(2*x2)^(2*x4))# exp(log(x6+x3-x1+1+1/5)+2.5*x1*x2^x4)
    Alpha = 0.1
    Lambda = 0.02
    Beta = rep(0,6)
    V = 0
...
Timevarying_nonlinear_gnrt <- function(N = 200, Distribution = "Exp", censor.rate = 1, npseu = 11, 
                                       partial = TRUE){
  ...
  while(Count <= N){
    ...
    if (Distribution == "Exp"){
      while(any(diff(sort(TS))<0.4)){
        TS <- sort(rtrunc(npseu-1, spec="beta", a=0.0001, b = 1, shape1 = 0.01, shape2 = 2))*200
      }
    } else if (Distribution == "WD"){
      while(any(diff(sort(TS))<0.4)){
        TS <- sort(rtrunc(npseu-1, spec="beta", a=0.0001, b = 1, shape1 = 0.001, shape2 = 2))*300
      }
    } else if (Distribution == "WI"){
      while(any(diff(sort(TS))<0.4)){
        TS <- sort(rtrunc(npseu-1, spec="beta", a=0.05, b = 10, shape1 = 0.7, shape2 = 2))*100
      }
    } else if (Distribution == "Gtz"){
      while(any(diff(sort(TS))<0.2)){
        TS <- sort(rtrunc(npseu-1, spec="beta", b = 5, shape1 = 0.2, shape2 = 10))*250+1
      }
    } else {
  ...
  if (npseu == 11){
    if(censor.rate == 0){
      ...
      if(Distribution == "Exp"){
        ...
        Censor.time = rexp(N,rate = 1/130)#67
      }
    }else if(censor.rate == 2){
      if(Distribution == "Exp"){
        Censor.time = rexp(N,rate = 1/24)
      }else if(Distribution == "WD"){
        Censor.time = rexp(N,rate = 1/30)
      }else if(Distribution == "WI"){
        Censor.time = rexp(N,rate = 1/45)
      }else if(Distribution == "Gtz"){
        Censor.time = rexp(N,rate = 1/76)#16
      }
    }else{
    ...
    if(censor.rate == 0){
      ...
      if(Distribution == "Exp"){
        ...
        Censor.time = rexp(N,rate = 1/275)
      }else if(Distribution == "WI"){
        Censor.time = rexp(N,rate = 1/150)
      }else if(Distribution == "Gtz"){
        Censor.time = rexp(N,rate = 1/187) # 47
      }
    }else if(censor.rate == 2){
      if(Distribution == "Exp"){
        Censor.time = rexp(N,rate = 1/27.5)
      }else if(Distribution == "WD"){
        Censor.time = rexp(N,rate = 1/40)
      }else if(Distribution == "WI"){
        Censor.time = rexp(N,rate = 1/56)
      }else if(Distribution == "Gtz"){
        Censor.time = rexp(N,rate = 1/16.2)
      }
    }else{
      stop("Wrong censoring type.")
    }
