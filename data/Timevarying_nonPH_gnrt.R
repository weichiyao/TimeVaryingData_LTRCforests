...
Comp_fstar_WI <- function(TALL, M, X){  # TALL = c(0,TS)
  ...
  tlen = length(TALL)
  if (M == "linear"){
    Beta <- c(1,1,1,1,10,1) #c(1,1,1,1,80,1)
    Fstar <- Beta[1]*x1+Beta[2]*x2+Beta[3]*x3+Beta[4]*x4+Beta[5]*x5+Beta[6]*x6
  } else if (M == "nonlinear"){
    Fstar = cos(x1+x3+x5+x6+x4+x2)#+log(x1+x3+x5+x6+x4+x2) #80*log(x6+x3+1/10)-x1*(x2*2)^(4*x4)
...
Timevarying_gnrt_nonPH_WI <- function(N = 200, model = c("linear","nonlinear","interaction"), 
                                      censor.rate = 1, npseu = 11, partial = TRUE){
  if (model == "linear"){
    Nadd = 100
  ...
  while(Count <= N){
    ...
    if (model == "linear") {
      while(any(diff(sort(TS)) < 0.2)){
        #TS <- sort(rtrunc(npseu-1, spec="beta", a=0.2, shape1 = 0.2, shape2 = 2))*10
  ...
  ##### ONLY CHECK FOR NPSEU = 11 CENSOR.RATE = 1!!!!!!
  if (npseu == 11){
    if (model == "linear"){
      if(censor.rate == 0){
  ...
  Data$X47 <- sample(c(1:5),n,replace = TRUE)
  Data$X60 <- sample(c(1:5),n,replace = TRUE)
  Data$X76 <- sample(c(1:5),n,replace = TRUE)
  Data$X15 <- runif(n,0,1)
  Data$X17 <- runif(n,0,1)
  Data$X23 <- runif(n,0,1)
  Data$X48 <- runif(n,0,1)
  Data$X61 <- runif(n,0,1)
  Data$X67 <- runif(n,0,1)
  Data$X77 <- runif(n,0,1)
  Data$X68 <- runif(n,1,2)
  Data$X69 <- runif(n,1,2)
  Data$X70 <- runif(n,1,2)
  Data$X79 <- runif(n,1,2)
  Data$X19 <- sample(c(0,1),n,replace=TRUE)
  Data$X51 <- sample(c(0,1),n,replace=TRUE)
  Data$X64 <- sample(c(0,1),n,replace=TRUE)
  Data$X78 <- sample(c(0,1),n,replace=TRUE)
  Data$X24 <- sample(c(0,1),n,replace=TRUE)
  Data$X25 <- sample(c(1:5),n,replace = TRUE)
  Data$X39 <- sample(c(0,1),n,replace=TRUE)
  Data$X7 = 0
  Data$X8 = 0
  Data$X31 = 0
  Data$X32 = 0
  ...
  for (Count in unique(Data$ID)){
    ...
    Data[Data$ID==Count,]$X31 <- runif(1,1,2)
    Data[Data$ID==Count,]$X32 <- runif(1,1,2)
    Data[Data$ID==Count,]$X42 <- runif(1,1,2)
    Data[Data$ID==Count,]$X55 <- runif(1,1,2)
    Data[Data$ID==Count,]$X72 <- runif(1,1,2)
    Data[Data$ID==Count,]$X9 <- sample(c(1:5),1,replace=TRUE)
    Data[Data$ID==Count,]$X33 <- sample(c(1:5),1,replace=TRUE)
    Data[Data$ID==Count,]$X43 <- sample(c(1:5),1,replace=TRUE)
    Data[Data$ID==Count,]$X56 <- sample(c(1:5),1,replace=TRUE)
    Data[Data$ID==Count,]$X73 <- sample(c(1:5),1,replace=TRUE)
    Data[Data$ID==Count,]$X10 <- runif(1,0,1)
    Data[Data$ID==Count,]$X11 <- sample(c(0,1),1,replace=TRUE)
    Data[Data$ID==Count,]$X44 <- sample(c(0,1),1,replace=TRUE)
    Data[Data$ID==Count,]$X57 <- sample(c(0,1),1,replace=TRUE)
    Data[Data$ID==Count,]$X74 <- sample(c(0,1),1,replace=TRUE)
    Data[Data$ID==Count,]$X12 <- sample(c(0,1,2),1,replace=TRUE)
    Data[Data$ID==Count,]$X45 <- sample(c(0,1,2),1,replace=TRUE)
    Data[Data$ID==Count,]$X58 <- sample(c(0,1,2),1,replace=TRUE)
    Data[Data$ID==Count,]$X75 <- sample(c(0,1,2),1,replace=TRUE)
    Data[Data$ID==Count,]$X21 <- runif(1,0,1)
    Data[Data$ID==Count,]$X41 <- runif(1,0,1)
    Data[Data$ID==Count,]$X54 <- runif(1,0,1)
    Data[Data$ID==Count,]$X71 <- runif(1,0,1)
    Data[Data$ID==Count,]$X22 <- sample(c(0,1),1,replace=TRUE)
    Data[Data$ID==Count,]$X34 <- sample(c(0,1),1,replace=TRUE)
    Data[Data$ID==Count,]$X26 <- sample(c(0,1,2),1,replace=TRUE)
    Data[Data$ID==Count,]$X35 <- sample(c(0,1,2),1,replace=TRUE)
    
    n0 = nrow(Data[Data$ID==Count,])
    x270 <- sample(c(0,1,2),1)
    x27 = cumsum(c(x270,sample(c(0,1),n0-1,replace = TRUE)))
    x27[x27>2]=2
    Data[Data$ID==Count,]$X27 = x27
    
    x400 <- sample(c(0,1,2),1)
    x40 = cumsum(c(x400,sample(c(0,1),n0-1,replace = TRUE)))
    x40[x40>2]=2
    Data[Data$ID==Count,]$X40 = x40
    
    x520 <- sample(c(0,1,2),1)
    x52 = cumsum(c(x520,sample(c(0,1),n0-1,replace = TRUE)))
    x52[x52>2]=2
    Data[Data$ID==Count,]$X52 = x52
    
    x650 <- sample(c(0,1,2),1)
    x65 = cumsum(c(x650,sample(c(0,1),n0-1,replace = TRUE)))
    x65[x65>2]=2
    Data[Data$ID==Count,]$X65 = x65
    
    x800 <- sample(c(0,1,2),1)
    x80 = cumsum(c(x800,sample(c(0,1),n0-1,replace = TRUE)))
    x80[x80>2]=2
    Data[Data$ID==Count,]$X80 = x80
    
    
    x13r <- sample(npseu-1,1)
    Data[Data$ID==Count,]$X13 <- c(numeric(x13r),rep(1,npseu-x13r))[1:n0]
    x28r <- sample(npseu-1,1)
    Data[Data$ID==Count,]$X28 <- c(numeric(x28r),rep(1,npseu-x28r))[1:n0]
    x46r <- sample(npseu-1,1)
    Data[Data$ID==Count,]$X46 <- c(numeric(x46r),rep(1,npseu-x46r))[1:n0]
    x59r <- sample(npseu-1,1)
    Data[Data$ID==Count,]$X59 <- c(numeric(x59r),rep(1,npseu-x59r))[1:n0]
    ...
    x291 <- sample(c(1,2),1)
    Data[Data$ID==Count,]$X29 <- c(rep(x291,x29r),rep(x291+1,npseu-x29r))[1:n0]
    x36r <- sample(npseu-1,1)
    x361 <- sample(c(1,2),1)
    Data[Data$ID==Count,]$X36 <- c(rep(x361,x36r),rep(x361+1,npseu-x36r))[1:n0]
    x49r <- sample(npseu-1,1)
    x491 <- sample(c(1,2),1)
    Data[Data$ID==Count,]$X49 <- c(rep(x491,x49r),rep(x491+1,npseu-x49r))[1:n0]
    x62r <- sample(npseu-1,1)
    x621 <- sample(c(1,2),1)
    Data[Data$ID==Count,]$X62 <- c(rep(x621,x62r),rep(x621+1,npseu-x62r))[1:n0]
    ...
    Data[Data$ID==Count,]$X37 <- c(rep(0,x37r[1]),rep(1,x37r[2]-x37r[1]),rep(2,npseu-x37r[2]))[1:n0]
    x38r <- sort(sample(npseu-1,2))
    Data[Data$ID==Count,]$X38 <- c(rep(0,x38r[1]),rep(1,x38r[2]-x38r[1]),rep(2,npseu-x38r[2]))[1:n0]
    x50r <- sort(sample(npseu-1,2))
    Data[Data$ID==Count,]$X50 <- c(rep(0,x50r[1]),rep(1,x50r[2]-x50r[1]),rep(2,npseu-x50r[2]))[1:n0]
    x63r <- sort(sample(npseu-1,2))
    Data[Data$ID==Count,]$X63 <- c(rep(0,x63r[1]),rep(1,x63r[2]-x63r[1]),rep(2,npseu-x63r[2]))[1:n0]
    k = runif(2)
    Data[Data$ID==Count,]$X20 <- k[1] * Data[Data$ID==Count,]$Start + k[2]
    Data[Data$ID==Count,]$X30 <- k[2] * Data[Data$ID==Count,]$Start + k[1]
    
    k2 <- runif(2,-2,-1)
    Data[Data$ID==Count,]$X53 <- k2[1] * Data[Data$ID==Count,]$Start + k2[2]
    Data[Data$ID==Count,]$X66 <- k2[2] * Data[Data$ID==Count,]$Start + k2[1]
  }
